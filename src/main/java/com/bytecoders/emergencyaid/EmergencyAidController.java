...
public class EmergencyAidController {
   ...
   *
   * @param query the search query. Can be UUID, phone number, or full name
   * @return list of matching patients or an empty list if no matches found
   */
  @GetMapping("/search/patients")
  public ResponseEntity<?> searchPatients(@RequestBody String query) {
    try {
      if (query == null || query.isBlank()) {
        return new ResponseEntity<>("Search query cannot be empty", HttpStatus.BAD_REQUEST);
      }
      // search for patients
   ...
   */
  @GetMapping("/patients/{pharmaId}/prescriptions")
  public ResponseEntity<?> getPatientPrescriptions(@PathVariable String pharmaId) {
    try {
      // logic for null pharmaId or empty prescriptions in PatientService
      List<Map<String, Object>> prescriptions = patientService.getPatientPrescriptions(pharmaId);
      return new ResponseEntity<>(prescriptions, HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<>("Unexpected error while retrieving prescriptions",
          HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}